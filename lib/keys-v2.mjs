...
export function chargeUsage({ row, model, inputTokens, outputTokens, logId }) {
  ...
  if (row.unlimited) {
    ...
    if (freeRemain >= cost) {
      ...
      source = "free";
    } else if (paidRemain >= cost) {
      CHARGE_PAID_STMT.run(cost, row.key_hash);
      source = "paid";
    } else if (row.balance_tokens >= cost) {
      CHARGE_BAL_STMT.run(cost, row.key_hash);
      source = "balance";
    } else {
      // Best-effort drain: free → paid → balance (allow negative balance, request already served).
      let remain = cost;
      const fromFree = Math.min(freeRemain, remain);
      if (fromFree > 0) { CHARGE_FREE_STMT.run(fromFree, row.key_hash); remain -= fromFree; }
      const fromPaid = Math.min(paidRemain, remain);
      if (fromPaid > 0) { CHARGE_PAID_STMT.run(fromPaid, row.key_hash); remain -= fromPaid; }
