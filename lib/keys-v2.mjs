...
const INSERT_LEDGER = db.prepare(`INSERT INTO usage_ledger (ts, key_hash, model, input_tokens, output_tokens, cost_tokens, source, log_id, usage_source, source_breakdown) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
export function chargeUsage({ row, model, inputTokens, outputTokens, logId, usageSource = "upstream" }) {
  ...
  INSERT_LEDGER.run(cst(), row.key_hash, model || null, inputTokens || 0, outputTokens || 0, cost, source, logId || null, usageSource, sourceBreakdown ? JSON.stringify(sourceBreakdown) : null);
