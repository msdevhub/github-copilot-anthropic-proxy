...
const INSERT_LEDGER = db.prepare(`INSERT INTO usage_ledger (ts, key_hash, model, input_tokens, output_tokens, cost_tokens, source, log_id, usage_source, source_breakdown, cache_read_tokens, cache_write_tokens) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
 ...
 */
export function chargeUsage({ row, model, inputTokens, outputTokens, cacheReadTokens = 0, cacheWriteTokens = 0, logId, usageSource = "upstream" }) {
  if (!row) return null;
  const cost = computeCost(model, inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens);
  let source = "unknown";
  ...
  withTransaction(() => {
    ...
    if (remain > 0) {
      ...
      drewOverdraft = remain;
      remain = 0;
    }

    const breakdown = { free: drewFree, paid: drewPaid, balance: drewBal, overdraft: drewOverdraft };
    sourceBreakdown = breakdown;
    const used = Object.entries(breakdown).filter(([, v]) => v > 0);
    if (used.length === 0) source = "unknown";
    else if (used.length === 1) source = used[0][0];
    else {
      // mixed: order by contribution desc for the human-readable summary
      used.sort((a, b) => b[1] - a[1]);
      source = "mixed:" + used.map(([k]) => k).join("+");
    }
  });

  INSERT_LEDGER.run(cst(), row.key_hash, model || null, inputTokens || 0, outputTokens || 0, cost, source, logId || null, usageSource, sourceBreakdown ? JSON.stringify(sourceBreakdown) : null, cacheReadTokens || 0, cacheWriteTokens || 0);
