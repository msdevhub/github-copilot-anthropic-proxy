...
// Maps token → { keyHash?: string, wxOpenid?: string }
//   - keyHash set: fully-logged-in user (API-key-bound session)
//   - wxOpenid set, keyHash null: WeChat user pending API-key bind
const USER_SESSIONS = new Map();

/** Returns the key_hash bound to a valid user_session cookie, or null.
 *  (Back-compat with old callers that only care about API-key-bound sessions.) */
...
/** Returns full session context: { token, keyHash, wxOpenid } or null. */
export function getUserSessionContext(req) {
  ...
  const ctx = USER_SESSIONS.get(tok);
  if (!ctx) return null;
  return { token: tok, keyHash: ctx.keyHash || null, wxOpenid: ctx.wxOpenid || null };
}

/** Creates a user session bound to a key_hash. Returns the cookie token. */
export function createUserSession(keyHash) {
  const token = randomBytes(24).toString("hex");
  USER_SESSIONS.set(token, { keyHash, wxOpenid: null });
  return token;
}

/** Creates a "pending" session bound to a wx openid (no api key yet). */
export function createWxPendingSession(openid) {
  const token = randomBytes(24).toString("hex");
  USER_SESSIONS.set(token, { keyHash: null, wxOpenid: openid });
  return token;
}

/** Promote a pending wx session to a fully-bound key_hash session (in place, same cookie). */
export function promoteUserSession(token, keyHash) {
  const ctx = USER_SESSIONS.get(token);
  if (!ctx) return false;
  ctx.keyHash = keyHash;
  USER_SESSIONS.set(token, ctx);
  return true;
}
