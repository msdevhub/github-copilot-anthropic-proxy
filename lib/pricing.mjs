...
function normalizeRates(body) {
  ...
  let cache_read_multiplier = null;
  let cache_write_multiplier = null;
  if (body.cache_read_multiplier != null && body.cache_read_multiplier !== "") {
    cache_read_multiplier = Number(body.cache_read_multiplier);
    if (!Number.isFinite(cache_read_multiplier) || cache_read_multiplier < 0) throw new Error("cache_read_multiplier must be non-negative number");
  }
  if (body.cache_write_multiplier != null && body.cache_write_multiplier !== "") {
    cache_write_multiplier = Number(body.cache_write_multiplier);
    if (!Number.isFinite(cache_write_multiplier) || cache_write_multiplier < 0) throw new Error("cache_write_multiplier must be non-negative number");
  }
  return { input_multiplier, output_multiplier, cache_read_multiplier, cache_write_multiplier };
}

export function setModelPricing(model, body) {
  if (!model) throw new Error("model required");
  const rates = normalizeRates(body);
  return registry.upsertPricing(model, rates.input_multiplier, rates.output_multiplier, rates.cache_read_multiplier, rates.cache_write_multiplier);
}
...
export function computeCost(model, inputTokens, outputTokens, cacheReadTokens = 0, cacheWriteTokens = 0) {
  ...
  const crT  = Math.max(0, Math.floor(Number(cacheReadTokens)  || 0));
  const cwT  = Math.max(0, Math.floor(Number(cacheWriteTokens) || 0));
  const inMult  = Number(r.input_multiplier)  || 0;
  const inMilli  = Math.round(inT  * inMult * 1000);
  const outMilli = Math.round(outT * (Number(r.output_multiplier) || 0) * 1000);
  const crMult = r.cache_read_multiplier  != null ? Number(r.cache_read_multiplier)  : inMult * 0.1;
  const cwMult = r.cache_write_multiplier != null ? Number(r.cache_write_multiplier) : inMult * 1.25;
  const crMilli = Math.round(crT * crMult * 1000);
  const cwMilli = Math.round(cwT * cwMult * 1000);
  const totalMilli = inMilli + outMilli + crMilli + cwMilli;
