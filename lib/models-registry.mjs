...
const SELECT_PRICING = db.prepare(`SELECT model_id, input_multiplier, output_multiplier, updated_at FROM model_pricing`);
const SELECT_PRICING_ONE = db.prepare(`SELECT input_multiplier, output_multiplier FROM model_pricing WHERE model_id = ?`);
const SELECT_DEFAULT_PRICING = db.prepare(`SELECT input_multiplier, output_multiplier FROM model_pricing WHERE model_id = '_default'`);
...
export function getDefaultRates() {
  return SELECT_DEFAULT_PRICING.get() || DEFAULT_RATES;
}

export function getRatesForModel(modelId) {
  if (!modelId) return getDefaultRates();
  const r = SELECT_PRICING_ONE.get(modelId);
  if (r) return { input_multiplier: r.input_multiplier, output_multiplier: r.output_multiplier };
...
export function getAllPricing() {
  ...
  for (const r of SELECT_PRICING.all()) {
    out[r.model_id] = { input_multiplier: r.input_multiplier, output_multiplier: r.output_multiplier, updated_at: r.updated_at };
  }
  return out;
}
...
const UPSERT_PRICING = db.prepare(`
  INSERT INTO model_pricing (model_id, input_multiplier, output_multiplier, updated_at)
  VALUES (?, ?, ?, ?)
...
export function upsertPricing(modelId, input_multiplier, output_multiplier) {
  ...
  UPSERT_PRICING.run(modelId, inMult, outMult, Date.now());
  return { input_multiplier: inMult, output_multiplier: outMult };
}
...
export function seedFromConfig({ legacyRegistry, legacyPricing }) {
  ...
  withTransaction(() => {
    ...
    if (!SELECT_DEFAULT_PRICING.get()) {
      UPSERT_PRICING.run("_default", DEFAULT_RATES.input_multiplier, DEFAULT_RATES.output_multiplier, now);
    }
  });
    ...
    for (const m of items) {
      ...
      if (isNew) {
        ...
        if (!SELECT_PRICING_ONE.get(id)) {
          UPSERT_PRICING.run(id, defaultRates.input_multiplier, defaultRates.output_multiplier, now);
        }
      } else {
        updated++;
      }
