...
const INSERT_LEDGER = db.prepare(
  'INSERT INTO usage_ledger (ts, key_hash, model, input_tokens, output_tokens, cost_tokens, source, log_id, usage_source, cache_read_tokens, cache_write_tokens) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
);
  ...
  try {
    ...
    if (isMonthlyActive(keyRow)) {
      ...
      INSERT_LEDGER.run(
        ...
        logEntry.usage?.cache_read || 0, logEntry.usage?.cache_write || 0
      );
    ...
    let inputTokens, outputTokens, cacheReadTokens = 0, cacheWriteTokens = 0;
    if (hasUpstreamUsage) {
      inputTokens = logEntry.usage.input || 0;
      outputTokens = logEntry.usage.output || 0;
      cacheReadTokens = logEntry.usage.cache_read || 0;
      cacheWriteTokens = logEntry.usage.cache_write || 0;
    } else {
