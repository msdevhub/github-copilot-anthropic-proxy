...
import { estimateCost, getRatesForModel } from "./pricing.mjs";
...
const INSERT_LEDGER = db.prepare(
  'INSERT INTO usage_ledger (ts, key_hash, model, input_tokens, output_tokens, cost_tokens, source, log_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
);

function windowRolloverIfNeeded(row) {
...
export function chargeFromLog(keyRow, logEntry, logId) {
  ...
  try {
    // ── Monthly plan: charge window_used ───────────────────────────────────
